<template>
  <div class="flex items-center justify-between my-5 px-4 lg:px-0">
    ...
    <div>
      <button @click="$router.push({name: 'cart'})" class="bg-main text-white p-2 rounded-lg">Корзина <span v-if="order != null" class="text-black font-bold">{{ order.goods.length }}</span></button>
...
export default {
  ...
  async mounted() {
    let res = await axios.get('https://627a34aa73bad506858480a8.mockapi.io/api/users');
    this.users = res.data
  },
  async created() {
  ...
  methods: {
    ...
    async registerUser() {
      await axios.post('https://627a34aa73bad506858480a8.mockapi.io/api/users', {
